...
def handle_client(client_socket):
    ...
    try:
        while True:
            ...
            try:
                msg_size = int(size_bytes.decode().strip())
            except ValueError:
                increment_stat("error_count")
                break

             # Read remaining bytes
            message_buffer = receive_n(client_socket, msg_size - 3)
            ...
            # Total length = 3-digit header + response body
            total_size = resp_len + 3
            # Format as 3 zero-padded digits
            size_str = f"{total_size:03d}"
            full_response = size_str + response
